...
client.on('data', (d) => {
  if (step === 0) {
    ...
    if (match) {
      ...
      qh.writeUInt16LE(1424, 14); // 1424: OPPlayBack/OPFileDownload
      qh.writeUInt32LE(dlPayload.length, 16);
      client.write(Buffer.concat([qh, dlPayload]));
    }
  } else if (step === 1) {
    console.log('HANDSHAKE ACCEPTED:', d.slice(20, 120).toString());
    step = 2;
    // Step 2: Send START command (msgId 1420: PLAY_REQ / Start)
    ...
    sh.writeUInt16LE(1420, 14); // 1420: START
    sh.writeUInt32LE(startPayload.length, 16);
    client.write(Buffer.concat([sh, startPayload]));
  } else if (step === 2) {
    totalBytes += d.length;
    console.log('RECEIVING VIDEO STREAM! Chunk len:', d.length, 'Total:', totalBytes);
    if (totalBytes > 200000) {
      console.log('SUCCESS! Downloaded over 200KB of video footage from DVR!');
      client.end();
    }
  }
});
...
setTimeout(() => { client.destroy(); process.exit(0); }, 6000);
